...
glmFindMaterial(GLMmodel* model, char* name)
{
    ...
    material (0). */
    animTcl::OutputMessage("glmFindMaterial():  can't find material \"%s\".\n", name);
    i = 0;
...
glmReadMTL(GLMmodel* model, char* name)
{
    ...
    if (!file) {
        animTcl::OutputMessage( "glmReadMTL() failed: can't open material file \"%s\".\n",
            filename);
        exit(1);
    }
    free(filename);
...
glmWriteMTL(GLMmodel* model, char* modelpath, char* mtllibname)
{
    ...
    if (!file) {
        animTcl::OutputMessage( "glmWriteMTL() failed: can't open file \"%s\".\n",
            filename);
        exit(1);
    }
    free(filename);
...
glmSecondPass(GLMmodel* model, FILE* file) 
{
  ...
  animTcl::OutputMessage(" Memory: %d bytes\n",
...
glmVertexNormals(GLMmodel* model, GLfloat angle)
{
    ...
    /* calculate an average normal for this vertex by averaging the
        ...
        node = members[i];
        if (!node)
            animTcl::OutputMessage( "glmVertexNormals(): vertex w/o a triangle\n");
...
#if 0
    animTcl::OutputMessage("glmLinearTexture(): generated %d linear texture coordinates\n",
        model->numtexcoords);
#endif
}
...
int GLMmodel::PartialCopy(GLMmodel *src)
{
	...
	if( src->GetNumVertices() > 0 )
	{
		...
		{
			animTcl::OutputMessage("GLMmodel: Error: cannot allocate memory for vertices.") ;
			return TCL_ERROR;
		}
	...
	if( src->GetNumNormals() > 0 )
	{
		...
		{
			animTcl::OutputMessage("GLMmodel: Error: cannot allocate memory for normals.") ;
			return TCL_ERROR;
		}
	...
	if( src->GetNumTexCoords() > 0 )
	{
		...
		{
			animTcl::OutputMessage("GLMmodel: Error: cannot allocate memory for texture coordinates.") ;
			return TCL_ERROR ;
		}
	...
	if( src->numfacetnorms > 0 )
	{
		...
		{
			animTcl::OutputMessage("GLMmodel: Error: cannot allocate memory for facet normals.") ;
			return TCL_ERROR;
		}
 ...
 */
int
GLMmodel::ReadOBJ(char* filename)
{
    FILE*   file;
	...
	if( filename == NULL ) 
	{
		animTcl::OutputMessage("ReadOBJ() failed: got a null pointer.") ;
		return TCL_ERROR ;
	}
    file = fopen(filename, "r");
    if (!file) {
		animTcl::OutputMessage("ReadOBJ() failed: can't open data file \"%s\".\n",
            filename);
        return TCL_ERROR;
    }
...
glmWriteOBJ(GLMmodel* model, char* filename, GLuint mode)
{
    ...
    if (mode & GLM_FLAT && !model->facetnorms) {
        animTcl::OutputMessage("glmWriteOBJ() warning: flat normal output requested "
            "with no facet normals defined.\n");
        mode &= ~GLM_FLAT;
    }
    if (mode & GLM_SMOOTH && !model->normals) {
        animTcl::OutputMessage("glmWriteOBJ() warning: smooth normal output requested "
            "with no normals defined.\n");
        mode &= ~GLM_SMOOTH;
    }
    if (mode & GLM_TEXTURE && !model->texcoords) {
        animTcl::OutputMessage("glmWriteOBJ() warning: texture coordinate output requested "
    ...
    if (mode & GLM_COLOR && !model->materials) {
        animTcl::OutputMessage("glmWriteOBJ() warning: color output requested "
            "with no colors (materials) defined.\n");
        mode &= ~GLM_COLOR;
    }
    if (mode & GLM_MATERIAL && !model->materials) {
        animTcl::OutputMessage("glmWriteOBJ() warning: material output requested "
            "with no materials defined.\n");
        mode &= ~GLM_MATERIAL;
    }
    if (mode & GLM_COLOR && mode & GLM_MATERIAL) {
        animTcl::OutputMessage("glmWriteOBJ() warning: color and material output requested "
    ...
    file = fopen(filename, "w");
    if (!file) {
        animTcl::OutputMessage( "glmWriteOBJ() failed: can't open file \"%s\" to write.\n",
            filename);
        exit(1);
    }
...
glmDraw(GLMmodel* model, GLuint mode)
{
    ...
    /* do a bit of warning */
    if (mode & GLM_FLAT && !model->facetnorms) {
        animTcl::OutputMessage("glmDraw() warning: flat render mode requested "
            "with no facet normals defined.\n");
        mode &= ~GLM_FLAT;
    }
    if (mode & GLM_SMOOTH && !model->normals) {
        animTcl::OutputMessage("glmDraw() warning: smooth render mode requested "
            "with no normals defined.\n");
        mode &= ~GLM_SMOOTH;
    }
    if (mode & GLM_TEXTURE && !model->texcoords) {
        animTcl::OutputMessage("glmDraw() warning: texture render mode requested "
            "with no texture coordinates defined.\n");
        mode &= ~GLM_TEXTURE;
    }
    if (mode & GLM_FLAT && mode & GLM_SMOOTH) {
    ...
    if (mode & GLM_COLOR && !model->materials) {
        animTcl::OutputMessage("glmDraw() warning: color render mode requested "
            "with no materials defined.\n");
        mode &= ~GLM_COLOR;
    }
    if (mode & GLM_MATERIAL && !model->materials) {
        animTcl::OutputMessage("glmDraw() warning: material render mode requested "
            "with no materials defined.\n");
        mode &= ~GLM_MATERIAL;
    }
    if (mode & GLM_COLOR && mode & GLM_MATERIAL) {
        animTcl::OutputMessage("glmDraw() warning: color and material render mode requested "
...
#if 0
    animTcl::OutputMessage("glmWeld(): %d redundant vertices.\n", 
...
glmReadPPM(const char* filename, int* width, int* height)
{
    ...
    if (strncmp(head, "P6", 2)) {
        animTcl::OutputMessage( "%s: Not a raw PPM file\n", filename);
        return NULL;
    }
...
if (model->numnormals) {
    ...
    animTcl::OutputMessage("glmOptimize(): %d redundant normals.\n", 
...
if (model->numtexcoords) {
    ...
    animTcl::OutputMessage("glmOptimize(): %d redundant texcoords.\n", 
